...
// Flexible Interface to catch whatever the backend sends
interface Order {
  id: number;
  user_id: string;
  status: string;
  ...
  // We allow all possibilities here to prevent errors
  items?: any[];
  OrderItems?: any[];
  order_items?: any[]; 
}
    ...
    <main className="min-h-screen bg-white font-sans pb-20">
      ...
      <div className="container mx-auto px-4 py-12 max-w-5xl">
        ...
        <div className="flex flex-wrap gap-4 mb-8 justify-center sm:justify-start">
          {["all", "processed", "shipped", "arrived"].map((tab) => (
             ...
             orders.map((order) => {
                ...
                return (
                    <AdminOrderCard
                        ...
                        description={firstItem.product?.description || "No description"}
                        quantity={firstItem.quantity}
                        price={firstItem.price_at_purchase}
                        imageSrc={firstItem.product?.image_url}
                        status={order.status}
                        onProceed={(id) => {
